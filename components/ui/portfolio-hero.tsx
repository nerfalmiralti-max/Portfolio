  ...
  return (
    <span ref={ref} className={`blur-text ${className}`} aria-label={text}>
      {segments.map((segment, index) => (
        <span aria-hidden="true" key={`${segment}-${index}`} style={{ transitionDelay: `${index * delay}ms`, opacity: inView ? 1 : 0, filter: inView ? "blur(0)" : "blur(10px)", transform: inView ? "translateY(0)" : "translateY(18px)" }}>
    ...
    <section className="hero" onPointerMove={move}>
      <div className="hero-copy">
        ...
        <div className="hero-actions">
          <Link href="/projects" className="button button-primary">{copy.hero.work}<ArrowRight size={17} /></Link>
          <Link href="/journey" className="button button-quiet">{copy.hero.journey}</Link>
        </div>
        <p className="age-note"><span />{profile.ageVisible ? copy.hero.note : "Building seriously."}</p>
      </div>
      <div className="orbit-system" ref={visualRef} aria-label="Three projects orbiting the AT monogram" role="img">
        <div className="coordinate coordinate-top">{profile.coordinates}</div>
        <div className="orbit-grid" />
        <div className="orbit orbit-one"><div className="project-node node-99"><span>01</span><strong>99 AKTAU</strong><small>SHIPPED</small></div></div>
        <div className="orbit orbit-two"><div className="project-node node-mangystau"><span>02</span><strong>MANGYSTAU</strong><small>ROUTES</small></div></div>
        <div className="orbit orbit-three"><div className="project-node node-kronos"><span>03</span><strong>KRONOS</strong><small>BUILDING</small></div></div>
        <div className="orbit-core"><span className="monogram monogram-hero" aria-hidden="true"><i>A</i><i>T</i></span><small>IDEA → REALITY</small></div>
